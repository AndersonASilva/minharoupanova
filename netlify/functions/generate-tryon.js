jsconst { GoogleGenerativeAI } = require('@google/generative-ai');
...
Generate a single photorealistic full-body image of the person wearing the garment.`;
    ...
    let msg = err.message || 'Erro ao chamar o Gemini.';
    if (err.message?.includes('API key')) msg = 'API Key inválida.';
    else if (err.message?.includes('SAFETY')) msg = 'Imagem bloqueada por segurança.';
    else if (err.message?.includes('quota')) msg = 'Limite de quota atingido.';
